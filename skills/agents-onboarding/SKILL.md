...
description: "Conduz a jornada de onboarding 'do zero ao agente de atendimento' do Indica Fácil Agents num VPS, escolhendo o orquestrador de deploy (Tier A Coolify, B Portainer, C compose genérico para VM crua ou qualquer painel). Provisiona DNS/SSH pelo MCP Hostinger, faz deploy de Chatwoot + Indica Fácil Agents + Langfuse com TLS, roda o /setup e importa o agente via MCP, pluga o Agent Bot do Chatwoot e valida ponta a ponta (playground + WhatsApp + traces no Langfuse). Também conduz a migração de uma secretária em n8n para a V4, lendo os workflows e convertendo o agente. Use quando o usuário quiser subir/onboardar uma instância nova do Indica Fácil Agents a partir do zero num VPS, em qualquer um desses orquestradores."
---

# Onboarding Indica Fácil Agents: do zero ao agente (multi-plataforma)

Leva uma VPS de "nada" até "agente de atendimento de IA rodando, testado e plugado numa caixa de entrada real". Esta skill é o **bundle executado pelo agente** (você): opera o VPS via SSH + a API do orquestrador escolhido (Coolify / Portainer / compose genérico), e controla o Indica Fácil Agents via **MCP** (OAuth, dry-run + audit).
...
1. **Leia [`guardrails.md`](guardrails.md) inteiro.** Tem fronteiras duras (só a VPS e o domínio indicados, MCP dry-run, nada de produção de terceiros, nada de segredo em log). Cruzar qualquer uma é parar e perguntar.
...
3. Confira pré-requisitos em [`references/00-prereqs-and-access.md`](references/00-prereqs-and-access.md): MCPs ligados (Hostinger ×3; não há hub central), acesso SSH e o contrato do ambiente.
...
| 4 | Deploy **Indica Fácil Agents** (edição Free/Pro pelo marcador, `templates/docker-compose.coolify.yml`, bootstrap 2-roles + migrate) | [`references/04-agents-image.md`](references/04-agents-image.md) |
| 5 | Deploy **Langfuse** (+ **MinIO S3 obrigatório**) | [`references/05-langfuse.md`](references/05-langfuse.md) |
| 6 | Indica Fácil Agents `/setup` (cria admin SUPER_ADMIN) → conectar **MCP** (OAuth) → **alvo de tenant** (`tenant_list`; passar `tenant` nas tools) | [`references/06-setup-and-mcp.md`](references/06-setup-and-mcp.md) |
| 8 | **Import do agente** (`agent_import`; padrão **Esther**/Clínica IndicaFácil.AI, vendorado em `samples/agents/esther-clinica-indicafacil.json`) + embedding por-tenant + reindex/retry da KB. **Havendo uma secretária em n8n, o agente sai dela** e não da Esther | [`references/08-agent-import.md`](references/08-agent-import.md) + [`references/migracao-v3.md`](references/migracao-v3.md) |
...
| 9 | Plugar Chatwoot no Indica Fácil Agents (`deployment_connect` → `set_accounts` → `inbox_bind`). **Havendo migração de n8n, isto é o cutover** e a ordem é obrigatória | [`references/09-chatwoot-bind.md`](references/09-chatwoot-bind.md) + [`references/migracao-v3.md`](references/migracao-v3.md) |
| 9b | **Ligar o Kanban no Chatwoot Pro**: com a imagem Pro no ar é **happy-path** (`enable-kanban` na conta). O sinal autoritativo é `enable-kanban` retornar `kanban_feature_enabled: true` (liga a feature na conta E só passa se a assinatura casar); **Refresh verde sozinho não basta**: imagem + assinatura + feature na conta, os três. Sem imagem Pro → OSS sem Kanban | [`references/chatwoot-kanban-enable.md`](references/chatwoot-kanban-enable.md) |
| 10 | Validar **E2E** (playground + grounding → **integração via Inbox API** → traces; **Kanban ativo** no tier Pro; WhatsApp real opcional). **Havendo migração, o agente já está em produção depois do cutover** | [`references/10-validate-e2e.md`](references/10-validate-e2e.md) + [`references/migracao-v3.md`](references/migracao-v3.md) |
...
O **usuário** cria o 1º admin no browser do orquestrador (Coolify/Portainer), do Chatwoot e do Indica Fácil Agents (`/setup`). Você **entrega o link + a instrução e espera** (no Coolify, `coolify.py wait-admin`; no Indica Fácil Agents, a URL `/setup`, que **não pede token**: o onboarding sobe com `SETUP_TOKEN_REQUIRED=false`), **nunca** cria essas contas por conta própria. Depois do admin criado, o token e o resto da config são com você. **Exceção: o Langfuse** é headless (`LANGFUSE_INIT_*`, etapa 5): você semeia a conta (usuário OWNER) e o operador só faz **login** (`/auth/sign-in`); como ele não vê o seed acontecer, faça um **handoff explícito** (anuncie o painel no ar, entregue URL + e-mail + senha temporária) e **espere ele confirmar que entrou** antes de seguir. Detalhes em `guardrails.md`.
