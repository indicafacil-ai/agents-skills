...
export function buildOnboardingEnv(opts: OnboardingEnvOptions): OnboardingEnv {
  ...
  const dbName = opts.dbName ?? "indicafacil_agents_db";
  const pgUser = opts.pgUser ?? "postgres";
  const appUser = opts.appUser ?? "indicafacil_app";
  ...
  if (!publicUrl) {
    console.error(
      "usage: bun scripts/gen-onboarding-env.ts --public-url https://agents.example.com [--out .env] [--force]\n" +
        "       [--db-host postgres] [--db-port 5432] [--db-name indicafacil_agents_db] [--pg-user postgres] [--app-user indicafacil_app]\n" +
