...
- `Permission denied (publickey…)` ou exit ≠ 0 → **sem acesso ainda**. Antes de gerar uma chave nova, tente a dedicada de uma run anterior: se `~/.ssh/indica-facil-agents` existe, sonde de novo com ela (`-i ~/.ssh/indica-facil-agents`). Logou → já está cadastrada, **reuse** (não gere outra nem peça re-paste). Senão, passo 2. (`BatchMode=yes` evita travar pedindo senha; `dangerouslyDisableSandbox: true` é obrigatório por ser rede.)
...
```sh
python3 scripts/sshkey.py wait-access --ssh root@<VPS_IP> --ssh-opts "-i ~/.ssh/indica-facil-agents"   # em background, non-blocking
```
`ok:true` → há acesso, siga. `ok:false` (timeout) → aí sim volte ao operador (ver troubleshooting abaixo). Daí use o **comando de trabalho** no resto do fluxo, com a chave que funcionou (a dedicada `~/.ssh/indica-facil-agents`, ou a existente que a sondagem achou no passo 1):
```sh
ssh -o IdentitiesOnly=yes -o IdentityAgent=none -o ConnectTimeout=12 -o BatchMode=yes \
    -o StrictHostKeyChecking=accept-new -i ~/.ssh/indica-facil-agents root@<VPS_IP>
```
...
2. **Chave errada apontada.** Confirme que o `-i` aponta pra chave cuja **pública** foi cadastrada (`~/.ssh/indica-facil-agents` se você gerou no passo 2). Se a sondagem do passo 1 achou uma chave existente, use **essa**, não a dedicada.
...
- `agents.<seu-dominio>`: Indica Fácil Agents
...
- **Sem VPS ainda?** Sugira adquirir (recomendado: [Hostinger](https://www.hostinger.com/br)). Detalhe na `00-prereqs-and-access.md`.
